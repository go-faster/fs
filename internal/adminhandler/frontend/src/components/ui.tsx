...
/**
 * Fullness bands. The thresholds mirror the placement watermarks: 0.9 is the
 * drain watermark a disk is taken out of placement at, 0.7 is where an operator
 * should already be planning capacity.
 */
export type Band = "healthy" | "degraded" | "critical";

export const DRAIN_WATERMARK = 0.9;

export function band(fullness: number): Band {
  return fullness >= DRAIN_WATERMARK ? "critical" : fullness >= 0.7 ? "degraded" : "healthy";
}

const BAND_THEMES: Record<Band, LabelProps["theme"]> = {
  healthy: "success",
  degraded: "warning",
  critical: "danger",
};

export function BandLabel({ band: b, children }: { band: Band; children: ReactNode }) {
  return <Label theme={BAND_THEMES[b]}>{children}</Label>;
}
...
/**
 * The message an API failure should show. The generated hooks type their error
 * as the spec's `{error_message}` body, but what the fetcher actually throws is
 * an `ApiError` carrying that text in `message` — read both, so neither shape
 * renders an empty box.
 */
export function errText(error: unknown): string {
  if (error instanceof Error && error.message) return error.message;
  if (error && typeof error === "object" && "error_message" in error) {
    const m = (error as { error_message?: unknown }).error_message;
