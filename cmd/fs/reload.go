...
func handleReload(ctx context.Context, lg *zap.Logger, configPath string, insecureNoAuth bool, authStore *auth.Store, srv *server.Server) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ch:
			reload(lg, configPath, insecureNoAuth, authStore, srv)
		}
	}
}
...
func reload(lg *zap.Logger, configPath string, insecureNoAuth bool, authStore *auth.Store, srv *server.Server) {
	...
	if authStore != nil {
		if ac, enabled, err := buildAuthConfig(cfg, insecureNoAuth); err != nil {
			...
			if err := authStore.Set(ac); err != nil {
