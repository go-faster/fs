...
## When adding functionality: observability, admin API, frontend

New functionality is not finished when it works. Three surfaces go stale
silently — nothing fails to compile, no test goes red, and the gap is found by
an operator during an incident or by a user staring at a dashboard that does not
mention the thing they are looking for.

Ask all three questions in the **same change**, and answer them in the PR even
...
**3. Frontend.** `internal/adminhandler/frontend` (Vite + React, typed client
generated by Orval from the same spec). After changing `_oas/admin.yml`, run
`npm run gen` there so the client matches, and surface the new state in the UI
if an operator would look for it. `npm run typecheck` must pass; CI runs it.
Do **not** commit the built `dist` bundle — it is built in CI and at release.

A change that adds an operator-visible capability and leaves all three untouched
...
- Adding a metric, an admin-API operation, or an admin-UI view → make sure the
