...
Three files, three jobs, and it is worth knowing which one to trust for what:

- [`known-failures.txt`](known-failures.txt) is the **gate** and the only
  authoritative statement. CI runs the whole suite on every PR and holds
  everything outside that file to passing, so it cannot drift.
- [`docs/CONFORMANCE.md`](../../docs/CONFORMANCE.md) is that file grouped by
  feature, generated from it by `make compat`. Also cannot drift.
- **This file is the *why***, and it is written by hand. Which failures are
  deliberate scope decisions, which are upstream tests nobody passes, which
  are bugs worth fixing next — none of that is derivable from a list of node
  IDs, which is exactly why it is prose.

The consequence is that **the counts below can go stale even while the gate is
green**: deleting a line from the list is what CI demands, and updating a
